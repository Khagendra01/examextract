...
def build_exam_docx(output_path, exam_data=None, content: Optional[List[tuple]] = None):
    """
    Build exam document from structured data or legacy content format.
    
    Args:
        output_path: Path (str) or file-like buffer (BytesIO) to save the .docx
    ...
    Usage:
        ...
        # Method 2: Using BytesIO buffer
        buf = BytesIO()
        build_exam_docx(buf, exam_data=exam)
        
        # Method 3: Using legacy content format
